...
{
	float prevParticleScale = 1.0f;
	public float particleScale = 1.0f;
	public bool alsoScaleGameobject = true;

	float prevScale = 1.0f;
	private float runtimeParticleScale;
	...
	void Start()
	{
		runtimeParticleScale  = particleScale * scaleMultiple;
		prevParticleScale = particleScale;
		// init particle system
		ParticleSystem[] mParticleSystems = GetComponentsInChildren<ParticleSystem>();
		foreach(ParticleSystem system in mParticleSystems)
		{
			system.Clear();
		}
	}

	public void SetscaleMultiple( float scale )
	{
		//Reset();
	...
	private void SetscaleMultiple2( float scale )
	{
		//Reset();
	...
	void Update () 
	{
		if (prevParticleScale != particleScale)
		{
			SetscaleMultiple2(particleScale);
			return;
		}

		//check if we need to update
		if (prevScale != runtimeParticleScale && runtimeParticleScale > 0)
		{
			if (alsoScaleGameobject)
				transform.localScale = new Vector3(runtimeParticleScale, runtimeParticleScale, runtimeParticleScale);

			float scaleFactor = runtimeParticleScale / prevScale;
			...
			prevScale = runtimeParticleScale;
		}
	}
