...
public class MeleeWeaponTrail : MonoBehaviour
{
    ...
    public void Init(Transform parent)
    {
		...
		_trailObject.transform.parent = parent;
		_trailObject.renderer.material = _material;
    }
	
	//对trailObject和trailMesh初始化
	private void InitTrailMesh()
	{
        ...
        _sizes = new float[] { -1f, 1f };
	}
	
    public void UpdateInfo(Transform start, Transform end, float trailTime, Color color)
    {
        _base = start;
        _tip = end;
        _lifeTime = trailTime;
            ...
            if (theDistanceSqr > _minVertexDistanceSqr)
            {
                ...
                if (make)
                {
                    ...
                    if (_points.Count >= 4)
                    {
                        ...
                        //GameDebuger.Log(" smoothTipList.Count: " + smoothTipList.Count);
                        ...
                        for (int n = 0; n < smoothTipList.Count; ++n)
                        {
                            ...
                            //else
                            //{
                            //	GameDebuger.LogError(idx + "/" + _smoothedPoints.Count);
                            //}
                        }
                    }
#endif
