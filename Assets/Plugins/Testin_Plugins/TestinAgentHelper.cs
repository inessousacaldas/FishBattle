...
public static class TestinAgentHelper
{
...
#if UNITY_EDITOR || UNITY_STANDALONE || !ENABLE_TESTIN
	...
	private static void _OnDebugLogCallbackHandler (string name, string stack, LogType type)
	{
		...
		try {
			cmasterReportException (TESTIN_CRASH_TYPE, name, stack);
    ...
    private static void LogCallback(string name, string stack, LogType type)
    {
        if (MainThreadID == Thread.CurrentThread.ManagedThreadId)
            _OnDebugLogCallbackHandler(name, stack, type);
    }

    private static void LogMultiThreadCallback(string name, string stack, LogType type)
    {
        if (MainThreadID != Thread.CurrentThread.ManagedThreadId)
            _OnDebugLogCallbackHandler(name, stack, type);
    }
