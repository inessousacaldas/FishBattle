...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        public List<PrefabPool> prefabPoolList
        {
            get { return _prefabPools; }
        }
        ...
        public void DestroyPrefabPool(PrefabPool prefabPool)
        {
            this._prefabPools.Remove(prefabPool);
            this.prefabs._Remove(prefabPool.prefab.name);

            //从SpawnPool的_spawned列表中移除
            foreach (var instance in prefabPool._spawned)
            {
        ...
        public void ClearDestroyItem()
        {
            ...
            {
                prefabPoolList[i].ClearDestoryItem();
            }
        }
        /// <summary>
...
#pragma warning restore
        ...
        public PrefabPool Despawn(Transform instance)
        {
...
#pragma warning restore
    ...
    public class PrefabPool
    {
        ...
        /// </description>
        public PrefabPool(Transform prefab, string bundleName = null , string assetName = null)
        {
        ...
        internal void SelfDestruct()
        {
            ...
            if (cullDespawnedCoroutine != null)
                this.spawnPool.StopCoroutine(cullDespawnedCoroutine);
            if (preloadOverTimeCoroutine != null)
                this.spawnPool.StopCoroutine(preloadOverTimeCoroutine);
        ...
        public int spawnedCount
        {
            get
            {
                //需要进行一次遍历,因为如果池对象没有归还池然后意外销毁了,_spawned列表中将会存在空对象
                int spawnCount = 0;
        ...
        /// <summary>
        /// 清理已经被外部Destroy的池对象引用
        /// </summary>
        public void ClearDestoryItem()
        {
            ClearDestroyItemByList(_spawned);
            ClearDestroyItemByList(_despawned);
        }

        private void ClearDestroyItemByList(List<Transform> list)
        {
        ...
        private Coroutine cullDespawnedCoroutine;
        private Coroutine preloadOverTimeCoroutine;
        ...
        internal bool DespawnInstance(Transform xform, bool sendEventMessage)
        {
                ...
                this.totalCount > this.cullAbove)   // Criteria met?
            {
                ...
                cullDespawnedCoroutine = this.spawnPool.StartCoroutine(CullDespawned());
            }
            if (this._spawned.Count == 0)
            {
                lastRefTime = Time.realtimeSinceStartup;
            }
            return true;
        }
        ...
        internal IEnumerator CullDespawned()
        {
            ...
            this.cullDespawnedCoroutine = null;
            yield return null;
        }
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            else
            {
                ...
                if (inst == null)
                {
                    var msg = "Make sure you didn't delete a despawned instance directly. PrefabName {0}";
                    Debug.LogError(string.Format(msg, this.prefab.name));
        ...
        internal void PreloadInstances()
        {
            ...
            if (this.preloadTime)
            {
                ...
                preloadOverTimeCoroutine = this.spawnPool.StartCoroutine(this.PreloadOverTime());
            }
            else
            {
