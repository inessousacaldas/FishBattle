...
namespace UniRx
{
    ...
    public static partial class ObservableExtensions
    {
        ...
        public static IDisposable SubscribeAndFire<T>(this IObservableExpand<T> source, Action<T> onNext)
        {
            try
            {
                return source.SubscribeAndFire(Observer.CreateSubscribeObserver(onNext, Stubs.Throw, Stubs.Nop));
            }
            catch(Exception e){
                CSGameDebuger.LogError(string.Format("SubscribeAndFire failed , e:{0}", e));
                return null;
            }
