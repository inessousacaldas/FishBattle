Shader "Blinn/Blinn-Phone-HightLevelScene"
{
	Properties
	{
        ...
        _SpecTint("SpecTint", Range(0, 1)) = 1
	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                half4 col = 0;
                fixed4 sET;
                //sET = tex2D(_Spec_Expo_Tint_Mask, i.uv);
                sET.x = tex2D(_Spec, i.uv);
                sET.y = tex2D(_Expo, i.uv);
                //sET.z = tex2D(_Tint, i.uv);
                half3 normal;
                ...
                half3 iSpec =  (_SpecTint * _SpecLightColor + (1 - _SpecTint) * difftex.rgb)* (pow(max(0, nh), sET.x * _SpecFactor) * _ExpoFactor * sET.y * sET.x);
                col.rgb += max(0, iSpec);
