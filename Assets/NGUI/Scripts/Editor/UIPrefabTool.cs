...
public class UIPrefabTool : EditorWindow
{
	...
	public void Reset ()
	{
		...
		if (mTab == 0)
		{
			...
			{
                if (s.EndsWith(".prefab") && s.Contains("Assets/UI/Prefabs/BaseUI"))// && s.Contains("Control -"))
					filtered.Add(s);
			}
...
//            path = path.Substring(path.IndexOf("Assets"));
            path = Regex.Match(path, "Assets" + @".*?$").Value;
...
#else
            if (File.Exists(path)){
                item.tex = (Texture2D)AssetDatabase.LoadAssetAtPath(path, typeof(Texture2D));
            }
            else{
                var prefab = item.prefab;
                if (prefab != null){
                    UISprite sp = prefab.GetComponentInChildren<UISprite>();    
                    if (sp != null && sp.atlas != null){
                        var atlas = sp.atlas;
                        var p = AssetDatabase.GetAssetPath(sp.atlas);
                        p = p.Replace(atlas.name + ".prefab", "imgs/" + sp.spriteName + ".png");
                        item.tex = File.Exists(p) ? (Texture2D)AssetDatabase.LoadAssetAtPath(p, typeof(Texture2D)): null;
                    }
                }
            }
