...
public class UIFontInspector : Editor
{
	...
	void OnSelectEmojiFont (Object obj)
	{
		mFont.emojiFont = obj as UIFont;
		NGUITools.SetDirty(mFont);
	}
	...
	public override void OnInspectorGUI ()
	{
		...
		//---EmojiFont Selector---
		if(mType == FontType.Bitmap || mType == FontType.Dynamic){
			GUILayout.Label ("EmojiFont");
			ComponentSelector.Draw<UIFont>(mFont.emojiFont, OnSelectEmojiFont, true);
			GUILayout.Space(6f);
		}
		//---EmojiFont Selector---
		...
		{
			DrawSymbolList (mFont);
		}

		if (mFont.isDynamic && mFont.emojiFont != null) {
			DrawSymbolList (mFont.emojiFont);
						}
	...
	private void DrawSymbolList(UIFont font){
		if (font.atlas != null)
		{
			if (NGUIEditorTools.DrawHeader("Symbols and Emoticons"))
			{
				...
				NGUIEditorTools.DrawSpriteField(font.atlas, mSymbolSprite, SelectSymbolSprite);
				...
				if (GUILayout.Button("Add", GUILayout.Width(80f)) && isValid)
				{
					NGUIEditorTools.RegisterUndo("Add symbol", font);
					font.AddSymbol(mSymbolSequence, mSymbolSprite);
					font.MarkAsChanged();
					mSymbolSequence = "";
					mSymbolSprite = "";
				}
				...
				//Add NamePrefix Symbol
				GUILayout.Space(10f);
				GUILayout.BeginHorizontal();
				mEmojiPrefix = EditorGUILayout.TextField("EmojiPrefix:",mEmojiPrefix, GUILayout.Width(220f));
				isValid = !string.IsNullOrEmpty(mEmojiPrefix);
				GUI.backgroundColor = isValid ? Color.green : Color.grey;

				if (GUILayout.Button("Auto Add Emoji", GUILayout.Width(120f)) && isValid)
				{
					NGUIEditorTools.RegisterUndo("Auto Add Emoji", font);
					List<UISpriteData> sprites = font.atlas.spriteList;
					for (int i = 0, imax = sprites.Count; i < imax; ++i)
					{
						UISpriteData sprite = sprites[i];
						if (sprite.name.StartsWith(mEmojiPrefix))
						{
							font.AddSymbol(sprite.name,sprite.name);
						}
					}
				...
				List<BMSymbol> symbols = font.symbols;
				...
				for (int i = 0; i < symbols.Count; )
				{
					...
					if (NGUIEditorTools.DrawSpriteField(font.atlas, sym.spriteName, ChangeSymbolSprite, GUILayout.MinWidth(100f)))
						mSelectedSymbol = sym;
					...
					if (GUILayout.Button("Edit", GUILayout.Width(40f)))
					{
						if (font.atlas != null)
						{
							NGUISettings.atlas = font.atlas;
							NGUISettings.selectedSprite = sym.spriteName;
							NGUIEditorTools.Select(font.atlas.gameObject);
						}
					}
					...
					{
						NGUIEditorTools.RegisterUndo("Remove symbol", font);
				...
				isValid = !string.IsNullOrEmpty(mSymbolSequence) && !string.IsNullOrEmpty(mSymbolSprite);
				GUI.backgroundColor = isValid ? Color.green : Color.grey;
