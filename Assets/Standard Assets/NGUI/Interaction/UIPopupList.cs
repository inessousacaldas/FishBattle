...
#endif
            _selectIdx = items.IndexOf(mSelectedItem);
            if (notify && _selectIdx >= 0)
                TriggerCallbacks();
            mSelectedItem = null;
        }
    }
    ...
    public virtual void Show()
    {
        if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
        {
            ...
            for (int i = 0, imax = items.Count; i < imax; ++i)
            {
                ...
                if (mSelectedItem == s || (i == CurSelectedItemIdx && string.IsNullOrEmpty(mSelectedItem)))
                    Highlight(lbl, true);
