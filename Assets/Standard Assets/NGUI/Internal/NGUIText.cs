...
static public class NGUIText
{
	...
	static public UIFont emojiFont;
	...
	static public void Update (bool request)
	{
		...
		useSymbols = (emojiFont != null && emojiFont.hasSymbols) && encoding && symbolStyle != SymbolStyle.None;
	...
	{
		return (emojiFont != null) ? emojiFont.MatchSymbol(text, index, textLength) : null;
	}
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color> cols,UIGeometry emoji)
	{
		if (string.IsNullOrEmpty(text)) return;

		int indexOffset = verts.size;
		int emojiOffset = emoji != null ? emoji.verts.size : 0;
		Prepare(text);
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				if (alignment != Alignment.Left)
				{
					...
					if (emoji != null)
					{
						Align(emoji.verts, emojiOffset, x - finalSpacingX);
						emojiOffset = emoji.verts.size;
					}
				}
			...
			if (symbol != null)
			{
				//表情的高度与字号一致，表情的宽度等比例缩小
				v0x = x + symbol.offsetX * fontScale;
				v1x = v0x + symbol.printWidth * fontScale;
				v1y = -(y + symbol.offsetY * fontScale);
				v0y = v1y - symbol.printHeight * fontScale;
				...
				if (Mathf.RoundToInt(x + symbol.advance * fontScale) > regionWidth)
				{
					...
					if (alignment != Alignment.Left)
					{
						...
						if (emoji != null)
						{
							if (emojiOffset < emoji.verts.size)
							{
								Align(emoji.verts, emojiOffset, x - finalSpacingX);
								emojiOffset = emoji.verts.size;
							}
				...
				if (emoji != null)
				{
					emoji.verts.Add(new Vector3(v0x, v0y));
					emoji.verts.Add(new Vector3(v0x, v1y));
					emoji.verts.Add(new Vector3(v1x, v1y));
					emoji.verts.Add(new Vector3(v1x, v0y));
				}
				...
				if (emoji != null && emoji.uvs != null)
				{
					...
					emoji.uvs.Add(new Vector2(u0x, u0y));
					emoji.uvs.Add(new Vector2(u0x, u1y));
					emoji.uvs.Add(new Vector2(u1x, u1y));
					emoji.uvs.Add(new Vector2(u1x, u0y));
				}

				if (emoji != null && emoji.cols != null)
				{
					if (symbolStyle == SymbolStyle.Colored)
					{
						for (int b = 0; b < 4; ++b) emoji.cols.Add(gc);
					}
					else
					{
						Color col = Color.white;
						col.a = gc.a;
						for (int b = 0; b < 4; ++b) emoji.cols.Add(col);
					}
				}
			}
