...
[AddComponentMenu("NGUI/Interaction/UIPageGrid")]
public class UIPageGrid : UIWidgetContainer
{
...
//	public bool hideInactive = false;
	...
	void OnValidate () { 
		maxRow = Mathf.Clamp(maxRow,1,int.MaxValue);
		maxCol = Mathf.Clamp(maxCol,1,int.MaxValue);
	...
	protected virtual void ResetPosition (List<Transform> list)
	{
		...
		for (int i = 0, imax = list.Count; i < imax; ++i)
		{
			...
			// See above
			//t.parent = myTrans;
			int page = i / (maxRow * maxCol);

			Vector3 pos = arrangement == Arrangement.Horizontal ? 
				new Vector3(cellWidth * y + page * maxCol * (cellWidth+pageGap), -cellHeight * x, 0f) : 
				new Vector3(cellWidth * y, -(cellHeight * x + page * maxRow *(cellHeight+pageGap)), 0f);
			...
			if (++y >= maxCol && maxCol > 0)
			{
				y = 0;
				if (++x >= maxRow && maxRow > 0)
				{
					x = 0;
				}
			}
