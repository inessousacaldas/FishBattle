...
public class UIAtlas : MonoBehaviour
{
	...
	public void SortDigital ()
	{
		mSprites.Sort(delegate(UISpriteData spriteA, UISpriteData spriteB) {
			int i = 0, j = 0;
			string nameA = spriteA.name;
			string nameB = spriteB.name;
			while (i < nameA.Length && j < nameB.Length) {
				if (char.IsDigit (nameA [i]) && char.IsDigit (nameB [j])) {
					string s1 = "", s2 = "";
					while (i < nameA.Length && char.IsDigit( nameA[i])) {
						s1 += nameA [i];
						i++;
					}
					while (j < nameB.Length && char.IsDigit( nameB[j] )) {
						s2 += nameB [j];
						j++;
					}
