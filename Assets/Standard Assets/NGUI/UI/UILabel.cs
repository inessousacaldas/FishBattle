...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] Overflow mOverflow = Overflow.ResizeFreely;
	...
	protected override void OnDisable ()
	{
        ...
        UICamera.onScreenResize -= UpdateNGUIText;
#endif
	...
	{
		if (mEmojiTex != null) mEmojiTex.geometry.Clear();
		if (!isValid) return;
		...
		int emojiStart = mEmojiTex != null ? mEmojiTex.geometry.verts.size : -1;

		UpdateNGUIText();

		NGUIText.tint = col;
		NGUIText.Print(text, verts, uvs, cols, mEmojiTex != null ? mEmojiTex.geometry : null);
		NGUIText.bitmapFont = null;
		NGUIText.dynamicFont = null;
		NGUIText.emojiFont = null;
		...
		if (mEmojiTex != null)
		{
			if (mEmojiTex.geometry.verts.size > emojiStart)
			{
				ApplyOffset(mEmojiTex.geometry.verts, emojiStart);
			}
		}
	...
	private UITexture mEmojiTex = null;

	private void UpdateEmoji()
	{
		if (mFont == null || !mEncoding || !mFont.isDynamic || mFont.emojiFont == null || !mText.Contains("#"))
		{
			CleanUpEmojiTexture();
			return;
		}
		...
		if (mEmojiTex == null)
		{
			Transform emojiChild = this.cachedTransform.Find("[emoticon]");
			if (emojiChild != null)
			{
				mEmojiTex = emojiChild.gameObject.AddMissingComponent<UITexture>();
			}
			else
			{
				mEmojiTex = NGUITools.AddWidget<UITexture>(this.cachedGameObject, 200);
				mEmojiTex.name = "[emoticon]";
			}
			mEmojiTex.updateAnchors = AnchorUpdate.OnUpdate;
			mEmojiTex.SetAnchor(this.cachedTransform);
			mEmojiTex.fillGeometry = false;
			//确保表情depth在最上层，减少UIPanel对UIDrawcall重建操作
			mEmojiTex.depth = 200;
		}

		mEmojiTex.material = mFont.emojiFont.material;
		mEmojiTex.pivot = this.pivot;
		//mEmojiTex.SetDimensions (this.mWidth,this.mHeight);
		mEmojiTex.MarkAsChanged();
		mEmojiTex.enabled = true;
	}

	private void CleanUpEmojiTexture()
	{
		if (mEmojiTex != null)
		{
			mEmojiTex.enabled = false;
		}
	}
	#endregion
}
