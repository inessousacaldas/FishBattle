...
{
	/// <summary>
	/// Current spriteAnimation that triggered the callback function.
	/// </summary>

	static public UISpriteAnimation current;
	...
	public UISprite sprite { get { return mSprite; } }
			...
			while (rate < mDelta)
			{
				...
				if (++frameIndex >= mSpriteNames.Count)
				{
					...
					if (!mLoop)
					{
						if (current != this)
						{
							UISpriteAnimation before = current;
							current = this;
