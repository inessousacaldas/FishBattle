...
namespace Pathfinding {
	...
	public class GridGraph : NavGraph, IUpdatableGraph
		...
		public class TextureData {
			...
			public float[] factors = new float[5];
			public ChannelUse[] channels = new ChannelUse[5];
			...
			public void Apply (GridNode node, int x, int z) {
				if (enabled && data != null && x < source.width && z < source.height) {
					Color col = data[z*source.width+x];
					...
					{
						//这里做个特殊处理,如果当前区域为行走区域了,就不设置当前区域为飞行区域了
						float val = col.g;
						if (col.r > 0) val = 0;
						ApplyChannel(node, x, z, val, channels[1], factors[1]);
					}
					...
					if (channels[3] != ChannelUse.None)
					{
						ApplyChannel(node, x, z, col.a, channels[3], factors[3]);
					}

					if (channels[4] != ChannelUse.None)
					{
						float all = col.r + col.g + col.b;
						ApplyChannel(node, x, z, all, channels[4], factors[4]);
					}
				}
			}

			/** Applies a value to the node using the specified ChannelUse */
			void ApplyChannel(GridNode node, int x, int z, float value, ChannelUse channelUse, float factor)
			{
				switch (channelUse)
				{
					...
					case ChannelUse.WalkablePenalty:
						...
						break;
					case ChannelUse.Tag:
						node.Tag = value > 0 ? (uint)factor : 0;
						break;
				}
			}
