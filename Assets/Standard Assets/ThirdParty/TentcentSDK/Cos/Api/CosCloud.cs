...
namespace QCloud.CosApi.Api
{
	...
	{
		string COSAPI_CGI_URL = "http://gz.file.myqcloud.com/files/v2/";
		//文件大于8M时采用分片上传,小于等于8M时采用单文件上传
		const int SLICE_UPLOAD_FILE_SIZE = 8 * 1024 * 1024;
		//用户计算用户签名超时时间
		const int SIGN_EXPIRED_TIME = 180;
		//HTTP请求超时时间
		const int HTTP_TIMEOUT_TIME = 20;
		private int appId;
		private string secretId;
		private string secretKey;
		private int timeOut;
		private QCloud.CosApi.Common.Request httpRequest;
        ...
        /// <param name="api_addr">Api的地址</param>
		public CosCloud(int appId, string secretId, string secretKey, string api_addr, int timeOut = HTTP_TIMEOUT_TIME)
		{
            this.COSAPI_CGI_URL = api_addr;
            this.appId = appId;
			this.secretId = secretId;
			this.secretKey = secretKey;
			this.timeOut = timeOut * 1000;
			this.httpRequest = new QCloud.CosApi.Common.Request();
		} 
        ...
        public string Upload(string bucketName,byte[] dataBuffer,string fileName,string remotePath, string bizAttribute = "", int insertOnly = 1)
        {
            var url = generateURL(bucketName, remotePath);
            var sha1 = SHA1.GetDataSHA1(dataBuffer);
            var data = new Dictionary<string, object>();
            data.Add("op", "upload");
            data.Add("sha", sha1);
            data.Add("biz_attr", bizAttribute);
            ...
            var header = new Dictionary<string, string>();
            header.Add("Authorization", sign);
            return httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, timeOut, fileName,dataBuffer);
        }
        /// <summary>
		                               ...
		                               string bizAttribute="", int sliceSize = SLICE_SIZE.SLIZE_SIZE_1M, int insertOnly = 1)
		{
            ...
            var obj = JsonMapper.ToObject(result);
            if((int)obj["code"] != 0)
            {
                return result; 
            }
            ...
            for (long offset = 0; offset < fileSize; offset+=sliceSize)
            {
                ...
                obj =  JsonMapper.ToObject(result);
                if((int)obj["code"] != 0)
                {
                    if (retryCount < 10)
                    {
		...
		{
			string url = COSAPI_CGI_URL + this.appId + "/" + bucketName + HttpUtils.EncodeRemotePath(remotePath) + UrlEncode(prefix);
			return url;
		}
        ...
        public static string UrlEncode(string str)
        {
            ...
            //string decodeString = utf8.GetString(encodedBytes);
            return str;
        }
		/// <summary>
		/// 内部方法：构造结果响应
