...
namespace Fish
{
    public static class OldBattleConfigConverter
    {
        ...
        public static Dictionary<int, CorrectSkillConfig> LoadConvertedBattleConfig()
        {
            ...
            return LoadConvertedBattleConfig(jsonTxt);
        }

        public static Dictionary<int,CorrectSkillConfig> LoadConvertedBattleConfig(string jsonTxt)
        {
        ...
        {
            var result=new List<CorrectSkillConfig>();
            foreach (var copy in oldLst)
            {
                if (copy.name!="普攻") continue;
                var newCfg = FromOld(copy);
                if (newCfg == null) continue;
                result.Add(newCfg);
            }
