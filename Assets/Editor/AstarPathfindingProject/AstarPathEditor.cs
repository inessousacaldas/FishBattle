	...
	void DrawSerializationSettings () {
		...
		if (fadeArea.Show()) {
			...
			if (GUILayout.Button("Save to file")) {
                string lastDir = "Assets/GameResources/ArtResources/SceneConfig/Navmesh/";//EditorPrefs.GetString("LastAstarDataDir", "");
                string path = EditorUtility.SaveFilePanel("Save Graphs", lastDir, "navmesh_" + SceneManager.GetActiveScene().name + ".bytes", "bytes");

				if (path != "") {
                    EditorPrefs.SetString("LastAstarDataDir", Path.GetDirectoryName(path));
#if ASTARDEBUG
			...
			{
                string lastDir = EditorPrefs.GetString("LastAstarDataDir", "");
                string path = EditorUtility.OpenFilePanel("Load Graphs", lastDir, "");
