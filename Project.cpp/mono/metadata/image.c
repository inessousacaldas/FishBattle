...
//char *cilu_dll_key() __attribute__((section (".cilu_magic_text")));
char *cilu_dll_key() {
    return "QHE8BxTiPWzMr8Je";
};

//这里就是.so初始化的时候，这里进行mytext断的解密工作
// void init_cilu_dll_key() __attribute__((constructor));
// unsigned long getLibAddr();
 
// void init_cilu_dll_key(){
//   char name[15];
//   unsigned int nblock;
//   unsigned int nsize;
//   unsigned long base;
...
void getNameOfPatch(const char *name, char *nameOfPatch) {
	...
	memset(packageName, 0, 256 * sizeof(packageName[0]));
	char dllName[100];
	pch = strtok(str, "/");
	while (pch != NULL) {
		if (!strncasecmp(pch, "com.", 4)) {
			// printf("found package name: %s\n", pch);
			// packageName = strtok(pch, "-");
			char* p = strstr(pch, "-");
			if(p) {
				strncpy(packageName, pch, (strlen(pch) - strlen(p)));
			} else {
				strncpy(packageName, pch, strlen(pch));
			}
		} else {
			if (ciluEndWithDLL(pch)) {
				// printf("found dll: %s\n", pch);
				strcpy(dllName, pch);
			}
		}
	...
	if (strlen(packageName) == 0 || strlen(dllName) == 0) {
		return;
	}

	// /data/data/com.cilugame.h13d/files/dlls/test1.dll
	char *fmt = "/data/data/%s/files/dlls/%s";
	sprintf(nameOfPatch, fmt, packageName, dllName);
}
