...
public class DLLUnZipTask extends AsyncTask<String, Integer, Integer> {
    ...
    public byte[] dz(byte[] orig, int step) {
        int tagLength = CILU_ZIP_DLL_MAGIC_TAG.length;
        byte[] magicTag = Arrays.copyOf(orig, tagLength);
        if (Arrays.equals(magicTag, CILU_ZIP_DLL_MAGIC_TAG)) {
            int keyLength = 32;
            byte[] zipKey = Arrays.copyOfRange(orig, orig.length-keyLength-1, orig.length-1);
            byte[] zipDllBytes = new byte[orig.length - tagLength - keyLength - 1];
            System.arraycopy(orig, tagLength, zipDllBytes, 0, zipDllBytes.length);
            zipDllBytes = e.xh(zipDllBytes, dk(zipKey, orig[orig.length-1]));
            // 1.
            publishProgress(step);
            byte[] unzipBytes = eb(zipDllBytes);
            // 2.
            publishProgress(step);
            byte[] encryptBytes = e.eh(unzipBytes, dk(Arrays.copyOfRange(zipKey, 16, keyLength), orig[orig.length-1]));
