...
public final class DeviceUtils {
		        ...
		        public void onReceive(Context context, Intent intent) {  
		            if (Intent.ACTION_BATTERY_CHANGED.equals(intent.getAction())) {  
		            	if(intent.getIntExtra("status", BatteryManager.BATTERY_STATUS_UNKNOWN) == BatteryManager.BATTERY_STATUS_CHARGING){
		            		UnityPlayer.UnitySendMessage("GameStart","OnPower","9999");
		            	}else{
			                int level = intent.getIntExtra("level", 0);  
			                int scale = intent.getIntExtra("scale", 100);  
			                int power = level * 100 / scale;
			                Log.d("Deom", "电池电量：:" + power);  
			                UnityPlayer.UnitySendMessage("GameStart","OnPower",Integer.toString(power));
		            	}
		            }else if(ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction())) {  
		            	UnityPlayer.UnitySendMessage("GameStart","OnNetworkChange",ConnectivityManager.CONNECTIVITY_ACTION);
		            }
		        }  
		    };  
	...
	private static class MyPhoneStateListener extends PhoneStateListener
	{
		public void onSignalStrengthsChanged(SignalStrength signalStrength)  
		{
			...
			UnityPlayer.UnitySendMessage("GameStart","OnGsmSignalStrength",Integer.toString(signalStrength.getGsmSignalStrength()));
		}
	}
	 ...
	 */
	 ...
	 {
		 	final int b = brightness;
			final Activity curActivity = getActivity();
			curActivity.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					 BrightnessUtil.setBrightness(curActivity, b); 
				}
			});
